<template>
  <div class="card relative">
    ...
    <div class="card-content pt-12">
      ...
      <!-- Global config -->
      <section class="rounded-lg border border-[hsl(var(--border))] p-4 space-y-4 mb-6">
        ...
        <div
          :class="{ 'pointer-events-none opacity-50 transition-opacity': !resolver.enabled }">
          <div class="setting-row !py-2">
            <div class="setting-label-group">
              <label class="setting-label">{{ t("rs_refresh_interval") }}</label>
              <p class="setting-desc">{{ t("rs_refresh_interval_desc") }}</p>
            </div>
            <div class="w-56 max-w-full">
              <select
                v-model.number="resolver.refresh_interval"
      ...
      <div :class="{ 'pointer-events-none opacity-50 transition-opacity': !resolver.enabled }">
      <nav
      ...
      <section v-show="activeSub === 'effective'" class="space-y-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <button class="btn btn-outline text-xs shrink-0" @click="exportResolverRules">
            ...
            <table class="w-full text-sm">
              <thead
                ...
                <tr class="border-b border-[hsl(var(--border))]">
                  <th class="text-left font-medium px-3 py-2">{{ t("rs_col_domain") }}</th>
                  <th class="text-left font-medium px-3 py-2">{{ t("rs_col_ip") }}</th>
                  <th class="text-left font-medium px-3 py-2">{{ t("rs_col_source") }}</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  ...
                  <td class="px-3 py-1.5 font-mono text-xs break-all">{{ r.domain }}</td>
                  <td class="px-3 py-1.5 font-mono text-xs">{{ r.ip }}</td>
                  <td class="px-3 py-1.5 text-xs text-[hsl(var(--muted-foreground))]">{{ r.source }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div
            ...
            <div class="flex items-center gap-2 px-3 py-2.5">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="text-xs text-[hsl(var(--muted-foreground))] truncate font-mono">
                  {{ sub.url }}
                </p>
              </div>
              <span class="text-[11px] text-[hsl(var(--muted-foreground))] shrink-0 hidden sm:block">
            ...
            <!-- Body (rules) -->
            <div v-if="expanded[idx]" class="border-t border-[hsl(var(--border))]">
              <div
                ...
                <table class="w-full text-sm">
                  <tbody>
                    <tr
                      ...
                      :key="i"
                      class="border-b border-[hsl(var(--border))] last:border-0">
                      <td class="px-3 py-1.5 pl-10 font-mono text-xs break-all">{{ r.domain }}</td>
                      <td class="px-3 py-1.5 font-mono text-xs">{{ r.ip }}</td>
                    </tr>
                  </tbody>
                </table>
