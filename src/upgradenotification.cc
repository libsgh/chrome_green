...
// `GetFileVersionInfo*` since chrome_green ships as `version.dll` and proxies
// the real one.
...
LSTATUS APIENTRY MyRegQueryValueExW(HKEY hKey,
  ...
  if (lpValueName && lstrcmpiW(lpValueName, L"pv") == 0) {
    ...
    // This hook fires once per `pv` query across the browser session and is
    // hit from every child process, so log it only once per process to keep
    // the debug log readable.
    static std::once_flag pv_logged;
    std::call_once(pv_logged, [&] {
      DebugLog(
...
LSTATUS APIENTRY MyRegOpenKeyExW(HKEY hKey,
  ...
  if (result != ERROR_SUCCESS && phkResult && lpSubKey &&
    ...
    if (RawRegOpenKeyExW(hKey, L"", 0, samDesired, phkResult) ==
        ERROR_SUCCESS) {
      // Substitution only happens once per process; log it once so the debug
      // log is not flooded by every child process's repeated key probes.
      static std::once_flag clients_subst_logged;
      std::call_once(clients_subst_logged, [] {
        DebugLog(
