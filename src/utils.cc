...
const std::wstring& GetIniPath() {
  static std::wstring ini_path = GetSelfDllDir() + L"\\chrome_green.ini";
  return ini_path;
}

// Get the directory where version.dll (this DLL) is located
...
std::wstring ExpandEnvironmentPath(const std::wstring& path) {
  ...
  DWORD need = ::ExpandEnvironmentStrings(
      path.c_str(), buffer.data(), static_cast<DWORD>(buffer.size()));
  if (need == 0) return {};
  if (need > buffer.size()) {
    buffer.resize(need);
    need = ::ExpandEnvironmentStrings(
        path.c_str(), buffer.data(), static_cast<DWORD>(buffer.size()));
    if (need == 0) return {};
  }
  // ExpandEnvironmentStrings() 的返回值含结尾 NUL，实际字符数 = need - 1。
  // 直接按返回值构造会多带一个内嵌 NUL，污染下游 wstring。
  return std::wstring(buffer.data(), need - 1);
}
...
void ExecuteCommand(int id, HWND hwnd) {
  ...
  // window gesture after close (#280).
...
void SendString(const std::wstring& str) {
  ...
  for (wchar_t ch : str) {
    ...
    down.ki.dwExtraInfo = GetMagicCode();
...
void LaunchCommands(const std::wstring& get_commands) {
  ...
  for (const auto& command : commands) {
    ...
    // https://github.com/Bush2021/chrome_plus/issues/130#issuecomment-2925782726 (upstream)
    ...
    std::wstring cmd =
        LR"(start "chrome_green cmd" cmd /c ")" + expanded_path + LR"(")";
    _wsystem(cmd.c_str());
  }
}
